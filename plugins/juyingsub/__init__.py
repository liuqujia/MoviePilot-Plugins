...
from .clients import PanSouClient, P115ClientManager, JuyingClient
...
class JuyingSub(_PluginBase):
    ...
    _juying_app_id: str = ""
    _juying_api_key: str = ""
    _juying_base_url: str = "https://share.huamucang.top"
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._juying_enabled = config.get("juying_enabled", True)
            self._juying_app_id = config.get("juying_app_id", "")
            self._juying_api_key = config.get("juying_api_key", "")
            self._juying_base_url = config.get("juying_base_url", "https://share.huamucang.top")
    ...
    def _init_clients(self):
        ...
        if self._juying_enabled:
            if not self._juying_app_id or not self._juying_api_key:
                logger.warning("聚影已启用但缺少 App ID 或 API Key，将无法使用聚影搜索功能")
                self._juying_client = None
            else:
                self._juying_client = JuyingClient(
                    app_id=self._juying_app_id,
                    api_key=self._juying_api_key,
                    proxy=proxy
                )
    ...
    def __update_config(self):
        self.update_config({
            ...
            "pansou_channels": self._pansou_channels,
            "juying_enabled": self._juying_enabled,
            "juying_app_id": self._juying_app_id,
            "juying_api_key": self._juying_api_key,
    ...
    def _do_sync(self) -> bool:
        # 至少启用一个搜索源
        if not self._pansou_enabled and not self._juying_enabled and not self._hdhive_enabled:
            logger.error("搜索源均未启用（PanSou/聚影/HDHive），无法执行")
            if self._notify:
                self.post_message(
                    mtype=NotificationType.Plugin,
                    title="【115网盘订阅追更】配置错误",
                    text="PanSou、聚影、HDHive 均未启用，请至少启用一个搜索源。"
                )
            return False
        ...
        try:
            if self._juying_client:
                self._juying_client.reset_api_call_count()
        except Exception:
            pass
        try:
