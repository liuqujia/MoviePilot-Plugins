...
# from ..utils import convert_juying_to_pansou_format  # JuYing client returns correct format directly
...
class SearchHandler:
    ...
    def get_enabled_sources(self) -> List[str]:
        ...
        :return: 搜索源名称列表，按 聚影 > HDHive > PanSou 排序
        """
        sources = []

        # 聚影
        if self._juying_enabled and self._juying_client:
            sources.append("juying")
    ...
    def search_single_source(
        ...
        :param source: 搜索源名称 ("juying", "hdhive", "pansou")
        :param mediainfo: 媒体信息
    ...
    def _search_juying(
        ...
        if not self._juying_client:
            logger.warning("聚影客户端未初始化，跳过聚影查询")
            return []
        ...
        if media_type == MediaType.MOVIE:
            ...
            keyword = f"{mediainfo.title} S{season}"

        logger.info(f"使用聚影搜索: {keyword}")
        try:
            results = self._juying_client.search_115_resources(keyword)
            if results:
                logger.info(f"聚影找到 {len(results)} 个资源")
                return results
